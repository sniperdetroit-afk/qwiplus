...
async function mountReputation(){
  ...
  async function loadTab(tab){
    ...
    const otherField = tab === "ventas" ? "reviewer_id" : "reviewed_id";
    ...
    listEl.innerHTML = list.map(r => {
      const name = escapeHtml(r.profiles?.name || "Usuario");
      const avatar = r.profiles?.avatar_url
        ? `<img src="${r.profiles.avatar_url}" style="width:44px;height:44px;border-radius:50%;object-fit:cover;">`
        : `<div style="width:44px;height:44px;border-radius:50%;background:rgba(34,211,238,0.2);border:1px solid rgba(34,211,238,0.3);display:flex;align-items:center;justify-content:center;color:#22d3ee;font-weight:700;font-size:16px;">${name.charAt(0).toUpperCase()}</div>`;
