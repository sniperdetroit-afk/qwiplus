  ...
  ">
    <div style="max-width:480px;margin:0 auto;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:32px;">
        ...
        >←</button>
        <h2 style="margin: 0;font-size: 24px;font-weight: 800;color: #F5F7FA;letter-spacing: -0.02em;">Configuración</h2>
      </div>

      <p style="font-size:11px;font-weight:800;color:#F5B942;letter-spacing:0.12em;text-transform:uppercase;margin:0 0 12px 4px;">⚡ Cuenta</p>
      ${settingsItem("✏️", "Editar perfil", "editProfile")}
      ${settingsItem("✅", "Verificación", "verification")}
      ${settingsItem("🔒", "Seguridad", "security")}

      <p style="font-size:11px;font-weight:800;color:#F5B942;letter-spacing:0.12em;text-transform:uppercase;margin:24px 0 12px 4px;">💳 Pagos</p>
      ${settingsItem("📦", "Dirección de envío", "shipping")}
      ${settingsItem("💳", "Métodos de pago", "payments")}

      <p style="font-size:11px;font-weight:800;color:#F5B942;letter-spacing:0.12em;text-transform:uppercase;margin:24px 0 12px 4px;">⚙️ Sistema</p>
      ${settingsItem("🔔", "Notificaciones", "notifications")}
      ${settingsItem("💡", "Sugerencias", "suggestions")}

      <p style="font-size:11px;font-weight:800;color:#F5B942;letter-spacing:0.12em;text-transform:uppercase;margin:24px 0 12px 4px;">🌐 Idioma</p>
      <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:4px 16px;backdrop-filter:blur(10px);">
        <select id="langSelect" style="width:100%;padding:14px 0;border:none;background:transparent;font-size:15px;font-weight:500;color:#F5F7FA;outline:none;cursor:pointer;">
          ${langOptions}
        </select>
      </div>
    </div>
...
function mountSettings() {
  ...
  if (back) back.onclick = () => navigate("profileMenu");
