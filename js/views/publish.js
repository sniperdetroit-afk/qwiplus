...
async function mountPublish() {
  ...
  if (params.category) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (params.subsub) {
          subSubCategorySelect.value = params.subsub;
        }
      }, 50);
    }, 50);
  }
  ...
  btn.addEventListener("click", async () => {
    ...
    if (!title) { showError("El título es obligatorio"); return; }
    if (!price) { showError("El precio es obligatorio"); return; }
    if (!categorySelect.value) { showError("Selecciona una categoría"); return; }
    ...
    try {
      ...
      console.error(err);
      showError("Error inesperado al publicar.");
    } finally {
      setLoading(false);
    }
  });
